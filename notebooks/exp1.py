...
def fill_missing_with_median(df):
    for column in df.columns:
        if df[column].isnull().any() or df[column].isna().any():  # Check if there are missing values in the column
...
X_train = train_processed_data.drop(columns=["Potability"], axis="columns")  # Features
